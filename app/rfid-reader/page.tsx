...
export default function ReaderPage() {
  ...
  const addOrUpdateTag = (
    ...
    const now = new Date();
    const currentAntennaFilter = antennaFilterRef.current;
    ...
    setTagDatabase((prev) => {
      ...
      if (newDb.has(epc)) {
        ...
        // Check if this antenna is allowed by filter
        const isAntennaAllowed = currentAntennaFilter === "all" || 
          currentAntennaFilter === antenna.toString();
        
        // Only update count if antenna is allowed by filter
        if (isAntennaAllowed) {
          tag.count++;
        }
        ...
        // Update antenna-specific count
        switch (antenna) {
          case 1:
            tag.antennaCounts.ant1++;
            break;
          case 2:
            tag.antennaCounts.ant2++;
            break;
          case 3:
            tag.antennaCounts.ant3++;
            break;
          case 4:
            tag.antennaCounts.ant4++;
            break;
        }
      } else {
        // New tag - check if antenna is allowed by filter
        const isAntennaAllowed = currentAntennaFilter === "all" || 
          currentAntennaFilter === antenna.toString();
  ...
  const getFilteredTags = (): TagData[] => {
    ...
    const tags = Array.from(tagDatabase.values()).filter((tag) => {
      ...
      if (antennaFilter && antennaFilter !== "all") {
        const filteredAntenna = parseInt(antennaFilter);
        const hasReadFromFilteredAntenna = 
          (filteredAntenna === 1 && tag.antennaCounts.ant1 > 0) ||
          (filteredAntenna === 2 && tag.antennaCounts.ant2 > 0) ||
          (filteredAntenna === 3 && tag.antennaCounts.ant3 > 0) ||
          (filteredAntenna === 4 && tag.antennaCounts.ant4 > 0);
        
        if (!hasReadFromFilteredAntenna) {
          return false;
        }
      }
  ...
  const isEpcVisibleInCurrentView = (epc: string): boolean => {
    ...
    // For antenna filter, check if tag has been read from the filtered antenna
    if (antennaFilterRef.current !== "all") {
      const filteredAntenna = parseInt(antennaFilterRef.current);
      const hasReadFromFilteredAntenna = 
        (filteredAntenna === 1 && tag.antennaCounts.ant1 > 0) ||
        (filteredAntenna === 2 && tag.antennaCounts.ant2 > 0) ||
        (filteredAntenna === 3 && tag.antennaCounts.ant3 > 0) ||
            ...
            <Table>
              <TableHeader className="sticky top-0 bg-card z-10">
                <TableRow>
                  ...
                  <TableHead className="text-center">Ant1</TableHead>
                  <TableHead className="text-center">Ant2</TableHead>
                  <TableHead className="text-center">Ant3</TableHead>
                  <TableHead className="text-center">Ant4</TableHead>
                  <TableHead
                    className="cursor-pointer"
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="text-center">
                          {tag.antennaCounts.ant1 > 0 ? (
                            <div className="bg-[#059669] text-white px-2 py-1 text-xs rounded font-semibold">
                              {tag.antennaCounts.ant1}
                            </div>
                          ) : (
                            <div className="bg-gray-200 text-gray-500 px-2 py-1 text-xs rounded">
                              0
                            </div>
                          )}
                        </TableCell>
                        <TableCell className="text-center">
                          {tag.antennaCounts.ant2 > 0 ? (
                            <div className="bg-[#059669] text-white px-2 py-1 text-xs rounded font-semibold">
                              {tag.antennaCounts.ant2}
                            </div>
                          ) : (
